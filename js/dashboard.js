...
function buildCSV(facilities) {
    ...
    // Summary section
    lines.push('Summary');
    lines.push('Facility,Type,Area (m2),Scope 1 (tCO2e),Scope 2 (tCO2e),Total (tCO2e)');
    for (const f of facilities) {
        ...
        lines.push(`${name},${type},${f.area || 0},${s1},${s2},${tot}`);
    }
...
export function renderDashboard(container, facilities, data) {
    const { scope1Total, scope2Total, grandTotal } = aggregateFacilities(facilities);
